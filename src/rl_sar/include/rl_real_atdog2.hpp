...
// atdog2（dog2）实机控制入口。
// 与 dog3 的主要区别：
//   - dog2 使用独立的 IMUDriver 读取姿态；dog3 复用 leg_driver 解析出的 IMU。
//   - dog2 为 12 自由度纯关节构型；dog3 为轮足构型（额外含 4 个轮子自由度）。
//   - dog2 上层速度限幅 vx/vz 为 ±2.5；dog3 为 ±1.5。
...
private:
    ...
    void SetCommand(const RobotCommand<float> *command) override; // 把策略输出下发到电机
    ...
    //real_port
    std::unique_ptr<IMUDriver> imu_driver;  // dog2 专用：独立 IMU 驱动
    std::unique_ptr<LegDriver> leg_driver;  // 腿部电机 USB 驱动（含 plane_dst）
