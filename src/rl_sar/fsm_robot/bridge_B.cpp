...
BridgeBStateAtdog2::BridgeBStateAtdog2(const std::string& urdf_file_path)
{
...
std::tuple<Vector3D, double> BridgeBStateAtdog2::get_robot_mass_info(
...
void BridgeBStateAtdog2::configure(
...
double BridgeBStateAtdog2::low_pass_filter(double input, int wheel_idx)
{
...
double BridgeBStateAtdog2::get_elapsed_time() const
{
...
void BridgeBStateAtdog2::reset_targets_to_current_pose()
{
...
RobotTarget BridgeBStateAtdog2::build_robot_target()
{
    ...
    if (bridge_stage == 2) {
        ...
        auto enforce_level_shank = [blend](LegTarget& leg) {
            // dog2 的 sagittal 链上，小腿水平约束可写成 q_thigh + q_calf = 0。
...
RobotTarget BridgeBStateAtdog2::update()
{
    const auto now = std::chrono::steady_clock::now();
