...
//   Check      检查姿态（dog2 专有，腿部摆到特定角度便于检修）
//   GetDown    趴下（插值回起始姿态）
...
//   mode==1 位控站立, 2 普通行走, 3 沙石, 4 台阶, 5 限高杆, 9 检查。
// 注：dog3 的状态机与此基本一致，但没有 Check 状态。
...
// dog2 状态机工厂：按名字创建状态实例，并向 FSMManager 注册（类型 "atdog2"）
