...
bool CDCTrans::open(uint16_t vid, uint16_t pid) {
    ...
    libusb_fill_bulk_transfer(
        recv_transfer, handle, EP_IN, cdc_rx_buffer, sizeof(cdc_rx_buffer),
...
int CDCTrans::send(const uint8_t* data, int size, unsigned int time_out) {
    int actual_size;
    if (_disconnected) {
        RCLCPP_ERROR(rclcpp::get_logger("cdc_device"), "\033[1;31m警告！！设备断连！！！！！！！\033[0m");
        return -2; // 设备断开
    }
    // 同步批量传输
    ...
    if (rc != 0) {
        RCLCPP_WARN(
            rclcpp::get_logger("package_comm"), "\033[1;31m发送数据失败！！！！！！！！发送数据%d,实际传输%d,返回%d\033[0m", size,
            actual_size, rc);
        return -1;
    } else
        return actual_size;
