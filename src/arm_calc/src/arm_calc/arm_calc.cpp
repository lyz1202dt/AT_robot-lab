...
PlanarIkGeometry ExtractPlanarIkGeometry(const KDL::Chain& chain) {
    ...
    geometry.tip_y_offset = tip.y() - geometry.base_xy.y();
...
JointVector ArmCalc::joint_pos_as(const CartesianPose& pose) {
    ...
    std::array<double, 2> q1_candidates{last_solution[0], last_solution[0]};
    std::size_t q1_candidate_count = 1;
    if (radial_distance > kEpsilon) {
        const double sin_q1_target = geometry.tip_y_offset / radial_distance;
        if (sin_q1_target < -1.0 - 1e-6 || sin_q1_target > 1.0 + 1e-6) {
            return last_solution;
        }

        const double target_angle = std::atan2(dy, dx);
        const double base_angle = std::asin(Clamp(sin_q1_target, -1.0, 1.0));
        q1_candidates[0] = NormalizeAngleNear(base_angle - target_angle, last_solution[0]);
        q1_candidates[1] = NormalizeAngleNear(kPi - base_angle - target_angle, last_solution[0]);
        q1_candidate_count =
            std::abs(NormalizeAngleNear(q1_candidates[1], q1_candidates[0]) - q1_candidates[0]) < 1e-8 ? 1 : 2;
    } else if (std::abs(geometry.tip_y_offset) > 1e-6) {
        return last_solution;
    }

    for (std::size_t q1_index = 0; q1_index < q1_candidate_count; ++q1_index) {
        const double q1 = q1_candidates[q1_index];
